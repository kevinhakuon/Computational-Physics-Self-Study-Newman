...
func = lambda x : x**3 / ((exp(x / (1 - x)) - 1) * (1 - x)**5)
...
def Stefan_Boltzmann (lower_bound = 0, upper_bound = 1, sample_point = 2000, resolution = 300):
     
    G_integral = GaussIntegrator(resolution)


    integral = G_integral.Gauss_quadrature(lower_bound, upper_bound)
    
    Energy_per_area = integral * k**4 / (4 * pi**2 * c**2 * hbar**3)
